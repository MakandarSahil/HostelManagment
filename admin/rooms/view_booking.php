...
<head>
  ...
  <style>
    ...
    table th { background-color: #3949ab; color: white; }
    ...
    .view-btn { background-color: #5bc0de; }
    ...
    .modal-content {
      background-color: #fff; margin: 8% auto; padding: 20px;
      border-radius: 8px; width: 400px; box-shadow: 0 5px 10px rgba(0,0,0,0.3);
    }
...
<body>
        ...
        <table id="bookingTable">
          ...
          <tbody>
            ...
            $sql = "SELECT rb.booking_id, CONCAT(u.fname, ' ', u.lname) AS student_name,
                           hr.room_no, rb.booking_date, rb.start_date, rb.end_date
                    FROM RoomBooking rb
            ...
            if ($result->num_rows > 0) {
              while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>
                          <button class='btn-action view-btn' onclick=\"viewBooking('{$row['student_name']}', '{$row['room_no']}', '{$row['booking_date']}', '{$row['start_date']}', '{$row['end_date']}')\">View</button>
                          <button class='btn-action edit-btn' onclick=\"editBooking('{$row['booking_id']}', '{$row['booking_date']}', '{$row['start_date']}', '{$row['end_date']}')\">Edit</button>
  ...
  <script>
    ...
    window.onclick = function (event) {
      const modals = ['bookingModal', 'editModal', 'viewModal'];
      modals.forEach(id => {
